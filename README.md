...
- **Synthetic NAV Computation** — DCF-based bond pricing across a weighted constituent basket (Treasuries + IG credit)
...
### Synthetic NAV
```
Portfolio Value = Σ (bond_price_i × weight_i)
NAV per share   = Portfolio Value / Shares Outstanding
```
...
## Yield Data
Treasury yields fetched via `yfinance` (^FVX, ^TNX, ^TYX). Credit spreads for IG constituents are applied on top of the relevant Treasury benchmark. In a production environment these would be sourced from Bloomberg or a fixed income data vendor.
